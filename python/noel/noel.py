...
class GiftMixer:
	...
	def mix(self):
		...
		while (1):
			...
			self.associations = zip(self.friends, temp_l)
			try:
...
def start_test():
	...
	giftMixer = GiftMixer(l)
	giftMixer.assert_associations(l, a_ok)
	try:
		giftMixer.assert_associations(l, a_nok1)
		raise BaseException("1")
	except AssertionError:
		pass
	try:
		giftMixer.assert_associations(l, a_nok2)
		raise BaseException("2")
	except AssertionError:
		pass
	try:
		giftMixer.assert_associations(l, a_nok3)
...
def create_list():
	...
	return  [ben, adri, agathe, nath, franz, dum, ge, verv, mkton, edouard, sarah, quentin]
